...
function getLaunchTime(){
 ...
 const timestamp=`${COPY.launchYear}-${pad(COPY.launchMonth)}-${pad(COPY.launchDay)}T${pad(COPY.launchHour)}:${pad(COPY.launchMinute)}:${pad(COPY.launchSecond)}${COPY.launchTimeZoneOffset}`;
 return new Date(timestamp);
}
function isBirthdayNow(now=new Date()){
 return now.getTime()>=getLaunchTime().getTime();
}
function show(id){
...
function updateCountdown(){
 ...
 birthdayUnlocked=now.getTime()>=launchTime.getTime();
 ...
 show('gate');
 const diff=Math.max(0,launchTime.getTime()-now.getTime());
 const values=[
